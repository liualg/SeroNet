...
def create_short(PMID, user_input_path=False):
	...
	if user_input_path == True:
		...
		if os.path.isdir(BASE_DIR):
			...
			try:
				df_path = glob(os.path.join(escape(BASE_DIR),'templated_data',f'PMID{PMID}*eviewed.xlsm'))[0]
			except:
				print("** NOT using reviewed files **")
				try:
					df_path = glob(os.path.join(escape(BASE_DIR), 'templated_data',f'PMID{PMID}*.xlsm'))[0]
		...
		if sub_section == 'study':
			...
			try:
				SUBJECT_ORGANISM = seroClass.subject_type_mode_organism(
					...
					df['SARS-CoV-2 Vaccine Type*'],
					df['COVID-19 Disease Severity*'],
					df[list(df.filter(regex=('Post.+')))[0]],
					df['COVID-19 Complications']
					)
			except:
				print('Using newer verion of organism section')
				# print(list(df.filter(regex=('Post.+')))[0])
				SUBJECT_ORGANISM = seroClass.subject_type_mode_organism(
					df['Arm ID'],
					df['Arm Name'],
					df['Study Population Description'],
					df['Arm Type'],
					df[list(df.filter(regex=('.+Species')))[0]],
					df['Biosample Type'],
					df['Strain Characteristics'],
					df['Sex at Birth*'],
					df['Age Event'],
					...
					df['COVID-19 Disease Severity*'],
					df[list(df.filter(regex=('Post.+')))[0]],
					# df[df.columns[df.columns.str.startswith('Post')]],
					df['COVID-19 Complications']
				)
